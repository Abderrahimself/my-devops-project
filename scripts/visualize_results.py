...
def parse_results(file_path):
    """Parse performance results JSON file"""
    with open(file_path, 'r') as f:
        data = json.load(f)
    return data

def generate_import_chart(results, output_file_prefix):
    """Generate import performance chart"""
    databases = []
    durations = []
    rates = []
    ...
    for result in results:
        ...
        durations.append(result.get("duration_seconds", 0))
        rates.append(result.get("records_per_second", 0))
    
    # Create figure with two subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 7))
    
    # Duration plot
    bars1 = ax1.bar(databases, durations, color=['blue', 'orange', 'green'])
    ...
    bars2 = ax2.bar(databases, rates, color=['blue', 'orange', 'green'])
    ax2.set_title('Import Speed by Database')
    ...
    plt.tight_layout()
    plt.savefig(f"{output_file_prefix}_import.png", dpi=300)
    plt.close()

def generate_query_chart(results, output_file_prefix):
    """Generate query performance chart"""
    # Group results by database and query type
    query_types = ["level_filter", "time_range", "complex_query"]
    databases = ["PostgreSQL", "MongoDB", "Elasticsearch"]
    ...
    for result in results:
        ...
        if db in data and qt in data[db]:
            data[db][qt] = result.get("avg_duration_seconds", 0)
    
    # Set up the data for plotting
    x = np.arange(len(query_types))  # the query types positions
    ...
    for i, db in enumerate(databases):
        values = [data[db].get(qt) if data[db].get(qt) is not None else 0 for qt in query_types]
        bar = ax.bar(x + (i - 1) * width, values, width, label=db)
        bars.append(bar)
    ...
    # Adjust layout and save
    plt.tight_layout()
    plt.savefig(f"{output_file_prefix}_query.png", dpi=300)
    plt.close()

def main():
    parser = argparse.ArgumentParser(description='Visualize database performance results')
    parser.add_argument('--input', required=True, help='Path to performance_results.json file')
    parser.add_argument('--output', default='performance_chart', help='Output file prefix (without extension)')
    args = parser.parse_args()
    
    # Parse results
    results = parse_results(args.input)
    
    # Generate import performance chart if available
    if "import_performance" in results:
        generate_import_chart(results["import_performance"], args.output)
    
    # Generate query performance chart if available
    if "query_performance" in results:
        generate_query_chart(results["query_performance"], args.output)
    
    # Generate combined chart
    plt.figure(figsize=(14, 10))
    plt.text(0.5, 0.5, "Performance Results", ha='center', va='center', fontsize=24)
    plt.axis('off')
