...
pipeline {
    ...
    stages {
        stage('Setup Environment') {
            steps {
                ...
                sh 'mkdir -p ${LOG_DIR}'
                sh 'mkdir -p ${JENKINS_LOG_DIR}'
                sh 'mkdir -p ${REPORTS_DIR}'
                
                // Set up the databases using your existing scripts
                sh 'bash scripts/db_scripts/setup_postgres.sh'
                sh 'bash scripts/db_scripts/setup_mongodb.sh'
                sh 'bash scripts/db_scripts/setup_elasticsearch.sh'
        ...
        stage('Generate Test Logs') {
            steps {
                // Generate synthetic test logs using your existing script
                sh 'python3 scripts/generate_test_logs.py --count 10000 --output ${LOG_DIR}/test_logs.json'
                sh 'ls -la ${LOG_DIR}'
            }
        }
        ...
        stage('Run Application') {
            steps {
                ...
                sh 'docker-compose -f ${DOCKER_COMPOSE_FILE} up -d app'
                
                // Allow some time for the application to generate logs
                sh 'sleep 30'
        ...
        stage('Collect Logs') {
            steps {
                // Copy application logs from Docker container to workspace
                sh 'docker cp app:/app/logs/app.json ${WORKSPACE}/${LOG_DIR}/ || true'
                sh 'docker cp app:/app/logs/app.log ${WORKSPACE}/${LOG_DIR}/ || true'
                
                // Collect Jenkins build logs
                sh 'mkdir -p ${JENKINS_LOG_DIR}'
                sh 'cp ${JENKINS_HOME}/jobs/${JOB_NAME}/builds/${BUILD_NUMBER}/log ${JENKINS_LOG_DIR}/jenkins_build.log || echo "No Jenkins log found"'
                
                // List all collected logs for verification
                sh 'ls -la ${LOG_DIR}'
                sh 'ls -la ${JENKINS_LOG_DIR} || echo "No Jenkins logs found"'
                
                // Archive logs as artifacts
                archiveArtifacts artifacts: "${LOG_DIR}/*/", fingerprint: true
                archiveArtifacts artifacts: "${JENKINS_LOG_DIR}/*/", fingerprint: true, allowEmptyArchive: true
            }
        }
        
        stage('Database Comparison') {
            steps {
                // Run the enhanced database comparison script with multiple log sources
                sh '''
                python3 scripts/import_logs.py \
                    --file ${LOG_DIR}/test_logs.json \
                    --app-logs "${LOG_DIR}/app.json" \
                    --text-logs "${LOG_DIR}/app.log" \
                    --jenkins-logs "${JENKINS_LOG_DIR}/*.log" \
                    --queries 20
                '''
                
                // Verify the results were generated
                sh 'ls -la performance_results.json'
        ...
        stage('Visualize Results') {
            steps {
                // Generate visualizations from results using your existing script
                sh 'python3 scripts/visualize_results.py --input performance_results.json --output ${REPORTS_DIR}/performance_chart'
                
                // Create a summary HTML report
                sh '''
                cat > ${REPORTS_DIR}/performance_report.html << EOF
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class="container">
                        ...
                        <img src="performance_chart_import.png" alt="Import Performance">
                        
                        <h2>Query Performance</h2>
                        <img src="performance_chart_query.png" alt="Query Performance">
                        
                        <p>This report compares the performance of PostgreSQL, MongoDB, and Elasticsearch for log storage and retrieval.</p>
                    </div>
                </body>
                </html>
                EOF
                ...
                // Archive visualizations and report
                ...
                publishHTML([
                    ...
                    reportTitles: 'Database Performance Comparison'
                ])
            }
        }
        ...
        stage('Analyze Results') {
            steps {
                ...
                echo "Top Performing Database for Imports:"
                cat performance_results.json | grep -A 2 "records_per_second" | sort -nr | head -3
                
                echo "Fastest Database for Queries:"
                cat performance_results.json | grep -A 2 "avg_duration_seconds" | sort -n | head -3
                '''
            }
        }
    }
    ...
    post {
        always {
            ...
            // Send notification
            echo 'Database performance comparison completed'
        }
        success {
